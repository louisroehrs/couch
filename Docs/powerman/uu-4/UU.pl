...
if ($#ARGV <= 1)
    {
    print STDERR 'Usage: UU <Host>[:port] <login:password> <[n]{on|off|pulse|status|power|name}|runNNN|interact> ...'."\n";
    exit -1;
    }
...
foreach (@ARGV)
	...
	while (<STDIN>)
		...
		{print "Commands: {?|help} | [n]{on|off|pulse|status|power|name} | runNNN | quit\n";}
	    elsif ($_ eq "quit")
		{last ;}
	    elsif (0==cmd($_))
	        {print "\t[OK]\n";}
...
#newer firmware is easier to parse
	...
	{
	while ($content =~ /<td.*?>([1-8])<\/td>\s*<td>(.*?)<\/td>[^\/]*?\W(ON|OFF)\W/igs)
	    {
	    if (($1 eq $n) || ($n eq 'a'))
		{
		if ($3 eq "ON")
    ...
    if (($1 eq '0') || ($n eq 'a'))
	{
	$content =~ /<th bgcolor="#DDDDFF" align=left>\s+Controller:\s+([^\<]*)/;
	$tmp=$1;
	$tmp =~ s/\s+$//;
	print "Controller: \"$tmp\"\n";
	}
    ...
    if ($n =~ /[1-4a]/)
	{
	$content =~ /<!--\s+RAW\s+VA=(\d+)\s+CA=(\d+)\s+VAH=(\S+)\s+CAH=(\S+)\s+(?:WHA100=(\S+)\s+)?-->/igs;
	$vah=((defined $3) && ($3>0)) ? $3.'V' : 'n/a';
	$cah=((defined $4) && ($4>0)) ? $4.'A' : 'n/a';
	$wha=((defined $5) && ($5>0)) ? ($5/10.).'kWh' : 'n/a';
    	print "Bus A: V=$vah I=$cah W=$wha\n";
        }
    if ($n =~ /[5-8a]/)
	{
	$content =~ /<!--\s+RAW\s+VB=(\d+)\s+CB=(\d+)\s+VBH=(\S+)\s+CBH=(\S+)\s+(?:WHB100=(\S+)\s+)?-->/igs;
	$vbh=((defined $3) && ($3>0)) ? $3.'V' : 'n/a';
	$cbh=((defined $4) && ($4>0)) ? $4.'A' : 'n/a';
	$whb=((defined $5) && ($5>0)) ? ($5/10.).'kWh' : 'n/a';
    	print "Bus B: V=$vbh I=$cbh W=$whb\n";
        }
    }
elsif (/^arun(\d{1,3})$/)
    {
